 ...
 * redeploy. Accounts that are already ADMIN are left alone; e-mails without an account are logged at WARN and skipped
 * (they become ADMIN when they register, see {@link UserService#registerUser}); an unset or empty variable is a
 * no-op. Only the e-mails and the count are logged, never passwords. The role takes effect at the account's next
 * login, because the access token carries the role. A lookup or save that fails is logged at ERROR and skipped rather
 * than ending the application (the routine is idempotent, so the next start retries it).
 * <p>
...
public class AdminBootstrap {
    ...
    public void promoteConfiguredAdmins() {
        ...
        for (var email : emails) {
            ...
            // Fix beyond the course: a listed e-mail with no account is the open register-first race described above, and
            // the INFO line below cannot tell it from "already ADMIN" - so it is named at WARN (an e-mail is logged when
            // it is promoted anyway, nothing new is exposed).
            try {
